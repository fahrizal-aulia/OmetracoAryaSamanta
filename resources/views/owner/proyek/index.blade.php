...
<div class="main-content" id="main-content">
    <div class="container">
        ...
        <form method="GET" action="{{ route('owner.proyek.index') }}" style="margin-bottom: 20px;">
    <input type="text" name="search" placeholder="Cari Nama Proyek..." value="{{ request('search') }}">
    <input type="date" name="tanggal_mulai" value="{{ request('tanggal_mulai') }}">
    <input type="date" name="tanggal_selesai" value="{{ request('tanggal_selesai') }}">
    <button type="submit" class="btn-search">Cari</button>
    <button type="button" class="btn-search reset" onclick="window.location.href='{{ route('owner.proyek.index') }}'">Reset</button>
</form>
            ...
            <tbody>
                @forelse($proyeks as $index => $proyek)
                    <tr>
                        ...
                        <td class="action">
                            <form action="{{ route('proyek.edit', $proyek->id) }}" method="GET">
                               
                            </form>
                            <form action="{{ route('proyek.destroy', $proyek->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus proyek ini?')">
                                @csrf
                                @method('DELETE')
