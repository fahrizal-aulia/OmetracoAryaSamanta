...
<div class="container">
    ...
    <div class="d-flex justify-content-end align-items-center mb-3">
        <a href="{{ route('projectcontrol.pembelian.view', ['id' => $proyekId, 'week' => $week - 1]) }}" class="btn btn-light me-3">←</a>
        <div>
            <div class="fw-bold">Minggu ke-{{ $week }}</div>
            <div>({{ $startOfWeek->translatedFormat('d M Y') }} – {{ $endOfWeek->translatedFormat('d M Y') }})</div>
        </div>
        <a href="{{ route('projectcontrol.pembelian.view', ['id' => $proyekId, 'week' => $week + 1]) }}" class="btn btn-light ms-3">→</a>
    </div>
</div>
